    ...
    class RFSCOMM
    {
	    ...
	    void send(uint8_t dest_id, uint8_t cmd, uint8_t val);
	    void send(uint8_t dest_id, uint8_t* data, uint8_t data_length);
            uint8_t receive();
            uint8_t rx_src_id();
            uint8_t rx_dev_id();
        ...
        private:
            uint8_t _my_id;
            uint8_t _src_id, _dev_id, _data_length;
            uint8_t _data[AES_BLOCK_SIZE-3];
